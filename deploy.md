...
Then on github.com create a new empty repo (no README/license) called e.g. `whatif`. Then:

```bash
git remote add origin https://github.com/YOUR-USERNAME/whatif.git
...
2. Connect the GitHub repo. Render detects `render.yaml` at the repo root and proposes a `whatif-api` web service.
3. Click **Apply**.
4. Render will fail the first deploy because the secret env vars are unset. Click on **whatif-api → Environment** and add:
...
6. When the service is "Live", note the URL — something like `https://whatif-api.onrender.com`. Hit `/health` to confirm:
   ```bash
   curl https://whatif-api.onrender.com/health
   # → {"status":"ok",…}
   ```
...
4. **Environment variables** — add one:
   ...
   | Key | Value |
   |---|---|
   | `VITE_API_URL` | `https://whatif-api.onrender.com/api/v1` (note the `/api/v1` suffix) |

5. Deploy. You'll get a URL like `https://whatif.vercel.app`.

---

## 6. Wire CORS (Render → Vercel)

The server's CORS allowlist comes from `FRONTEND_URL`. Go back to **Render → whatif-api → Environment** and set:

```
FRONTEND_URL = https://whatif.vercel.app
```
